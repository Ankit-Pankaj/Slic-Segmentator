 ...
 * API
 * ---
 *	let segmentator = new SLICSegmentator()
 *    	segmentator.SLICSegmentation(imageURL, options)
 *
...
class SLICSegmentator {
    ...
    computeSLICSegmentation(imageData, options) {
        var imWidth = imageData.width,
            ...
            labData = this.xyz2lab(this.rgb2xyz(imageData.data,
                imageData.width,
                imageData.height),
                imageData.width,
                imageData.height);
        // Compute edge.
        this.computeEdge(labData, edgeMap, imageData.width, imageData.height);
    ...
    computeSegmentation(imageData, options) {
        var segmentation = this.computeSLICSegmentation(imageData, options),
            numSegments = this.remapLabels(segmentation);
        if (options.callback) {
        ...
        if (options.toDataURL)
            ...
            this.getDataURL(imageData.width, imageData.height, segmentation, options);
    }

    // Convert to Data URL.
    getDataURL(width, height, indexMap, options) {
    ...
    onSuccessImageLoad(image, options) {
        var canvas = document.createElement('canvas');
        ...
        var imageData = context.getImageData(0, 0, image.width, image.height),
            segmentation = this.computeSegmentation(imageData, options);
    }
    ...
    SLICSegmentation(imageURL, options) {
        ...
        // imageURL = imageURL + '?dummy-url'
        image.src = imageURL;
        image.onError = () => { this.onErrorImageLoad(image); }
        image.onload = () => { this.onSuccessImageLoad(image, options); }
        // image.onerror = function () { _this.onErrorImageLoad(image); };
        // image.onload = function () { _this.onSuccessImageLoad(image, options); };
    };
}
export default SLICSegmentator
